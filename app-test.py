...
class BasicTestCase(unittest.TestCase):
    def test_index(self):
        """initial test, if Flask was set up correctly"""
        tester=app.test_client(self)
        response = tester.get('/', content_type='html/text')
        #self.assertEqual(response.status_code, 404)
        self.assertEqual(response.status_code, 200)
        #self.assertEqual(response.data, b'Hello, cruel world!')
...
class FlaskrTestCase(unittest.TestCase):
    ...
    def setUp(self):
        """ setup blank temp db before each test"""
    ...
    def tearDown(self):
        """ destrong blank temp db after each test"""
        db.drop_all()
    ...
    def test_delete_message(self):
        """ Ensure all messages are being deleted"""
